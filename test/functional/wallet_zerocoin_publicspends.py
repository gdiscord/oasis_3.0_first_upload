...
class ZerocoinSpendTest(OasisTestFramework):
    ...
    def run_test(self):
        ...
        def check_balances(denom, zxos_bal, xos_bal):
            zxos_bal -= denom
            assert_equal(self.nodes[2].getzerocoinbalance()['Total'], zxos_bal)
            xos_bal += denom
            wi = self.nodes[2].getwalletinfo()
            assert_equal(wi['balance'] + wi['immature_balance'], xos_bal)
            return zxos_bal, xos_bal
        ...
        zxos_balance = self.nodes[2].getzerocoinbalance()['Total']
        assert_equal(balance, DecimalAmt(13833.92))
        assert_equal(zxos_balance, 6666)
        ...
        zxos_balance, balance = check_balances(denom_2, zxos_balance, balance)
        ...
        block_time = stake_4_blocks(block_time)
        self.check_tx_in_chain(0, txid)
        zxos_balance, balance = check_balances(denom_3, zxos_balance, balance)
        ...
        _, _ = check_balances(denom_1, zxos_balance, balance)
        self.log.info("--> VALID PUBLIC COIN SPEND (v3) PASSED")
