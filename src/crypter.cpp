    ...
    if(IsCrypted()) {
        if (!IsLocked()) { //if we have password
            ...
            if (EncryptSecret(vMasterKey, kmSeed, hashSeed, vchSeedSecret)) {
                ...
                if (db.WriteZXOSSeed(hashSeed, vchSeedSecret)) {
                    return true;
                }
            }
            strErr = "encrypt seed";
        ...
        if (db.WriteZXOSSeed(hashSeed, ToByteVector(seed))) {
            return true;
        }
        strErr = "save zxosseed to wallet";
    }
                //the use case for this is no password set seed, mint dzXOS,
...
bool CCryptoKeyStore::GetDeterministicSeed(const uint256& hashSeed, uint256& seedOut)
{
    ...
    if (IsCrypted()) {
        if(!IsLocked()) { //if we have password
            ...
            std::vector<unsigned char> vchCryptedSeed;
            //read encrypted seed
            if (db.ReadZXOSSeed(hashSeed, vchCryptedSeed)) {
        ...
        // wallet not crypted
        if (db.ReadZXOSSeed(hashSeed, vchSeed)) {
