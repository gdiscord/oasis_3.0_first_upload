                       ...
                       int &nPaymentCount, int &nBlockStart, CBitcoinAddress &address, CAmount &nAmount)
{
    ...
    if (nAmount < 10 * COIN)
        throw JSONRPCError(RPC_INVALID_PARAMETER, strprintf("Invalid amount - Payment of %d is less than minimum 2 XOS allowed", FormatMoney(nAmount)));
...
UniValue preparebudget(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 6)
        throw std::runtime_error(
            "preparebudget \"proposal-name\" \"url\" payment-count block-start \"oasis-address\" monthy-payment\n"
            ...
            "5. \"oasis-address\":   (string, required) OASIS address to send payments to\n"
            ...
            "\nExamples:\n" +
            HelpExampleCli("preparebudget", "\"test-proposal\" \"http://oasisco.in/t/test-proposal\" 2 820800 \"oWBxLjXs5ywmmses1utCRqR4xidGFeV9aT\" 500") +
            HelpExampleRpc("preparebudget", "\"test-proposal\" \"http://oasisco.in/t/test-proposal\" 2 820800 \"oWBxLjXs5ywmmses1utCRqR4xidGFeV9aT\" 500"));
    ...
    CWalletTx wtx;
    if (!pwalletMain->GetBudgetSystemCollateralTX(wtx, budgetProposalBroadcast.GetHash(), useIX)) { // 2 XOS collateral for proposal
...
UniValue submitbudget(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 7)
        throw std::runtime_error(
            "submitbudget \"proposal-name\" \"url\" payment-count block-start \"oasis-address\" monthly-payment \"fee-tx\"\n"
            ...
            "\nExamples:\n" +
            HelpExampleCli("submitbudget", "\"test-proposal\" \"http://oasisco.in/t/test-proposal\" 2 820800 \"oWBxLjXs5ywmmses1utCRqR4xidGFeV9aT\" 500") +
            HelpExampleRpc("submitbudget", "\"test-proposal\" \"http://oasisco.in/t/test-proposal\" 2 820800 \"oWBxLjXs5ywmmses1utCRqR4xidGFeV9aT\" 500"));
...
UniValue getbudgetprojection(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw std::runtime_error(
            ...
            "    \"PaymentAddress\": \"xxxx\",     (string) OASIS address of payment\n"
