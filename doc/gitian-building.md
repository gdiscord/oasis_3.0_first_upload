...
*Setup instructions for a gitian build of OASIS Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the OASIS
...
to the OASIS GitHub release page.
...
- [Building OASIS Core](#building-oasis-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/oasis-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/oasis-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/oasis-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/oasis-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/oasis-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/oasis-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/oasis-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@oasis.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/oasis-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/oasis-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/oasis-osx-*-build.assert
```
